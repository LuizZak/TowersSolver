extension Sequence {
    ...
    @inlinable
    public func onlyTwo(where predicate: (Iterator.Element) throws -> Bool) rethrows -> (first: Element, second: Element)? {
        var first: Element?
    ...
    @inlinable
    public func count(_ count: Int, where predicate: (Iterator.Element) throws -> Bool) rethrows -> Bool {
        var _count = 0
