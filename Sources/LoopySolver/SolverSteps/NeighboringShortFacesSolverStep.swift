...
private class InternalSolver {
    ...
    private func apply(to pair: FacePair) {
        ...
        let applyToEdges: (Edge.Id, Edge.Id) -> Void = {
            ...
            guard
                (grid.faceContainsEdge(face: pair.face1, edge: $0)
                    || grid.faceContainsEdge(face: pair.face1, edge: $1))
                    && (grid.faceContainsEdge(face: pair.face2, edge: $0)
                        || grid.faceContainsEdge(face: pair.face2, edge: $1))
            else {
                return
            }
            ...
            if count1 >= (grid.hintForFace(pair.face1) ?? Int.max)
                && count2 >= (grid.hintForFace(pair.face2) ?? Int.max)
            {
