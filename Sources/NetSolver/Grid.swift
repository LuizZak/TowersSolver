...
public struct Grid: GridType, Equatable {
    ...
    public func areNeighbors(atColumn1 column1: Int, row1: Int, column2: Int, row2: Int) -> Bool {
        guard isWithinBounds(column: column1, row: row1),
            isWithinBounds(column: column2, row: row2)
        else {
            return false
        }
    ...
    public func columnRowByMoving(column: Int, row: Int, direction: EdgePort) -> (
        column: Int, row: Int
    ) {
        var column = column
        var row = row
    ...
    /// coordinates.
    public func edgePort(from tile1: (column: Int, row: Int), to tile2: (column: Int, row: Int))
        -> EdgePort?
    {
        guard
            areNeighbors(
