        ...
        for row in 0..<grid.rows {
            for column in 0..<grid.columns {
                ...
                let reduced =
                    tile
                    .orientations(
                        excludingPorts: unavailableIncomingPortsForTile(atColumn: column, row: row)
                    )
                    .filter {
                        required.isSubset(of: Tile.portsForTile(kind: tile.kind, orientation: $0))
                    }

                for orientation in metadata.possibleOrientations(column: column, row: row)
...
extension SolverInvocation: NetSolverDelegate {
    ...
    func requiredPortsForTile(atColumn column: Int, row: Int) -> Set<EdgePort> {
        ...
        let surrounding = EdgePort.allCases.filter { edgePort in
            ...
            let neighborOrientations = metadata.possibleOrientations(
                column: neighborCoordinates.column,
                row: neighborCoordinates.row
            )
            let neighborCommonAvailable = neighbor.commonAvailablePorts(
                orientations: neighborOrientations
            )
