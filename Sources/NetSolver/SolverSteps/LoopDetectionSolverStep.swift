...
struct LoopDetectionSolverStep: NetSolverStep {
    ...
    func analyzeNetwork(_ network: Network,
                        grid: Grid,
                        delegate: NetSolverDelegate) -> [GridAction] {
        ...
        let tiles: [OpenPortTile] = openPorts.flatMap { openPort in
            openPort.ports.map {
                let coordinate = grid.columnRowByMoving(column: openPort.column, row: openPort.row, direction: $0)
                
                let tile = grid[row: coordinate.row, column: coordinate.row]
                
                return OpenPortTile(column: coordinate.column,
                                    row: coordinate.row,
                                    ports: tile.ports,
                                    originTile: openPort)
            }
        ...
        for i in 0..<(tiles.count - 1) {
            ...
            for j in (i + 1)..<tiles.count {
                ...
                if grid.areNeighbors(atColumn1: tile1.column, row1: tile1.row,
                                     column2: tile2.column, row2: tile2.row) {
    ...
    func analyzeNeighbors(_ tile1: OpenPortTile,
        ...
        guard let tile1PortToTile2 = grid.edgePort(from: (tile1.column, tile1.row),
                                                   to: (tile2.column, tile2.row)) else {
            return []
        }
        
        let tile1Orientations =
            delegate.possibleOrientationsForTile(atColumn: tile1.column,
                                                 row: tile1.row)
        ...
        return [
            .markImpossibleOrientations(column: tile1.column,
                                        row: tile1.row,
