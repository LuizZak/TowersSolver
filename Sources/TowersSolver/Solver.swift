...
public class Solver {
    ...
    private func runWhileChangingGrid(_ closure: () throws -> Void) rethrows {
        var beforeGrid: [Cell]
        repeat {
    ...
    private func runStepAcrossLines(
        _ step: (
            _ visible: Int, _ towers: [Cell], _ convertCoord: (_ index: Int) -> (x: Int, y: Int)
        ) throws -> Void
    ) rethrows {
        ...
        for (x, v) in grid.visibilities.bottom.enumerated() {
            ...
            try step(
                v,
                grid.column(x: x, orientation: .lookingUp),
                { (x: x, y: grid.size - 1 - $0) }
            )
        }
        ...
        for (y, v) in grid.visibilities.right.enumerated() {
            ...
            try step(
                v,
                grid.row(y: y, orientation: .lookingFromRight),
                { (x: grid.size - 1 - $0, y: y) }
            )
        }
    }
        ...
        if visible > 1 && visible < towers.count {
            ...
            for max in (1..<visible).reversed() {
                ...
                if interactive && descriptive && _set {
                    _print(
                        "[\(visible)] At \(_coord): Removing impossible \(r) tower heights for visibility \(visible)"
                    )
                }
    ...
    private func fillTrivial(visible: Int, towers: [Cell], set: (Int, Int) -> Void) {
            ...
            if towers.last?.solution == towers.count {
                ...
                if interactive && descriptive {
                    if towers[0].solution != towers.count - 1 {
                        _print(
                            "[\(visible)] At \(_coord): Blocking view of towers up to \(towers.count) with \(towers.count - 1) tower"
                        )
                    }
                }
            ...
            if towers.first?.solution == 1 {
                ...
                if interactive && descriptive {
                    if towers[1].solution != towers.count {
                        _print(
                            "[\(visible)] At \(_coord): Smallest tower must be followed by largest tower"
                        )
                    }
                }
        ...
        if visible > 1 && towers[visible - 1].solution == towers.count {
            ...
            if Set(sol) == Set(visible...towers.count) {
                ...
                if interactive && descriptive {
                    _print(
                        "[\(visible)] At \(_coord): Adding sequential tower heights (due to being only solution possible)"
                    )
                }
        ...
        if visible > 1 && visible < towers.count && !towers[visible - 1].hasSolution {
            if towers[0..<visible - 1].solutionHeights() == Array(1..<visible) {
                ...
                if interactive && descriptive {
                    _print(
                        "[\(visible)] At \(_coord): Adding tallest tower after sequential tower heights (due to being only solution possible)"
                    )
                }
        ...
        if visible == towers.count {
            ...
            if interactive && descriptive {
                _print(
                    "[\(visible)] At \(_coord): Only solution is sequential tower heights from 1 - \(towers.count)"
                )
            }
        ...
        for (i, cell) in towers.enumerated() {
            ...
            case .hint(let hints) where hints.count == 1:
                ...
                if interactive && descriptive {
                    _print(
                        "[\(visible)] At \(_coord): Sole hint \(hints.first!) is the only solution for cell at \(i + 1)!"
                    )
                }
            default:
                break
        ...
        for h in available {
            ...
            if c.count == 1 {
                ...
                if interactive && descriptive {
                    _print(
                        "[\(visible)] At \(_coord): Cell \(c[0].0 + 1) is the only cell on its row featuring height hint \(h): That's its solution."
                    )
                }
            }
        }
    ...
    private func complexStep(
        ...
        for (i, possibleSet) in possibleSets.enumerated() {
            ...
            if possibleSet.count == 1 {
                ...
                if interactive && descriptive {
                    _print(
                        "[\(visible)] At \(_coord): Cell \(i + 1) only has \(possibleSet.first!) in common across possible options"
                    )
                }
            }
            ...
            if let hints = towers[i].hints {
                ...
                if diff.count > 0 {
                    ...
                    if interactive && descriptive {
                        let list = Array(diff).sorted().map { h in h.description }.joined(
                            separator: ", "
                        )
                        _print(
                            "[\(visible)] At \(_coord): Cell \(i + 1) cannot feature height(s) \(list) since that would result in empty cells elsewhere!"
                        )
                    }
                }
            }
...
extension Solver {
    ...
    public func solve() -> Bool {
        ...
        while guesses < maxGuessAttempts && guesses < possible.count {
            ...
            subSolver.markSolvedCell(
                x: least.offset % subGrid.size,
            ...
            if subSolver.solve() {
                ...
                if interactive {
                    _print(
                        "A Guess attempt resulted in invalid solution. Backtracking (or quitting)..."
                    )
                }
            }
