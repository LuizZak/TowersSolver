...
class MockNetSolverDelegate: NetSolverDelegate {
    ...
    var mock_guaranteedOutgoingUnavailablePortsForTile:
        ((_ column: Int, _ row: Int) -> Set<EdgePort>)?
    ...
    func enqueue(_ step: NetSolverStep) {
        precondition(
            _didPrepare,
            "Invoke mock_prepare(forGrid:) before using \(MockNetSolverDelegate.self)"
        )
    ...
    func guaranteedOutgoingAvailablePortsForTile(atColumn column: Int, row: Int) -> Set<EdgePort> {
        return baseSolverDelegate.guaranteedOutgoingAvailablePortsForTile(
            atColumn: column,
            row: row
        )
    }
    ...
    func guaranteedOutgoingUnavailablePortsForTile(atColumn column: Int, row: Int) -> Set<EdgePort>
    {
        ...
        return baseSolverDelegate.guaranteedOutgoingUnavailablePortsForTile(
            atColumn: column,
            row: row
        )
    }
