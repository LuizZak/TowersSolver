...
class SignpostGridPrinterTests: XCTestCase {
    ...
    func testPrintGrid_3x3_unsolved_connections() {
        ...
        let gen = SignpostGridGenerator(rows: 3, columns: 3)
        gen.loadFromGameID("1deecaaag9a")
        gen.grid[column: 1, row: 0].connectionState = .connectedTo(.init(column: 1, row: 2))
        gen.grid[column: 0, row: 2].connectionState = .connectedTo(.init(column: 0, row: 1))
        let sut = SignpostGridPrinter(bufferWidth: 20, bufferHeight: 11)
        sut.target = target
        sut.printGrid(grid: gen.grid)
    ...
    func testPrintGrid_3x3_unsolved_connections_skipsNumberedSequences() {
        ...
        let gen = SignpostGridGenerator(rows: 3, columns: 3)
        gen.loadFromGameID("1deecaaag9a")
        gen.grid[column: 0, row: 0].connectionState = .connectedTo(.init(column: 1, row: 1))
        gen.grid[column: 0, row: 2].connectionState = .connectedTo(.init(column: 0, row: 1))
        let sut = SignpostGridPrinter(bufferWidth: 20, bufferHeight: 11)
        sut.target = target
        sut.printGrid(grid: gen.grid)
    ...
    func testPrintGrid_3x3_unsolved_connectionsWithNumbers_forwards() {
        ...
        gen.loadFromGameID("1deecaaag9a")
        gen.grid[column: 0, row: 0].connectionState = .connectedTo(.init(column: 1, row: 1))
        gen.grid[column: 1, row: 1].connectionState = .connectedTo(.init(column: 1, row: 0))
        let sut = SignpostGridPrinter(bufferWidth: 20, bufferHeight: 11)
        sut.target = target
        sut.printGrid(grid: gen.grid)
    ...
    func testPrintGrid_3x3_unsolved_connectionsWithNumbers_backwards() {
        ...
        let gen = SignpostGridGenerator(rows: 3, columns: 3)
        gen.loadFromGameID("1deecaaag9a")
        gen.grid[column: 2, row: 0].connectionState = .connectedTo(.init(column: 2, row: 2))
        gen.grid[column: 2, row: 1].connectionState = .connectedTo(.init(column: 2, row: 0))
        let sut = SignpostGridPrinter(bufferWidth: 20, bufferHeight: 11)
        sut.target = target
        sut.printGrid(grid: gen.grid)
